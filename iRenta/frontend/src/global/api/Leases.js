...
export const fetchLeases = async () => {
  ...
  export const fetchLeaseById = async (leaseId) => {
    ...
    try {
      ...
      console.error("Error fetching lease by ID:", err.response?.data || err.message);
  ...
  export const updateLease = async (leaseId, updatedData) => {
    ...
    try {
      const response = await axios.put(`${API_BASE_URL}/${leaseId}`, updatedData, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });
      ...
      console.error("Error updating lease:", error.response?.data || error.message);
