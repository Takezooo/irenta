...
const RentPayments = () => {
  ...
  const loadPayments = async () => {
    try {
      ...
      // Filter payments for current tenant if needed
      setPayments(data);
    } catch (error) {
  ...
  const loadRentDates = async () => {
    try {
      const leaseId = "current-lease-id"; // Get from auth context or props
      const data = await fetchRentDatesByLease(leaseId);
      setRentDates(data);

      // // Check for upcoming rent dates and notify
      // checkUpcomingRentDates(data);
    } catch (error) {
      console.error('Error loading rent dates:', error);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const selected = rentDates.find(date => date._id === e.target.value);
              ...
              >
                ...
                {rentDates
                  .filter(date => !date.payment) // Only show unpaid rent dates
                  .map(date => (
                    <option key={date._id} value={date._id}>
                      {new Date(date.rentDate).toLocaleDateString()} - {new Date(date.endDate).toLocaleDateString()}
                      {` (₱${date.baseAmount})`}
                    </option>
                  ))}
              </select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          ...
          <p className="text-xl font-bold text-green-500">
            {payments.some(p => p.status === 'Pending') ? 'Pending' : 'Current'}
          </p>
        </div>
      </div>
            ...
            <tbody>
              {isLoading ? (
                ...
                payments.map((payment) => (
                  <tr key={payment._id} className={`border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
