...
const CreateLease = ({ seekerId }) => {
  ...
  const calculateTotalRent = () => {
    const { baseRent, utilities, parking, amenities, otherFees } =
      formData.contractDetails.rentBreakdown;
    const total = [baseRent, utilities, parking, amenities].reduce(
      (acc, val) => acc + (parseFloat(val) || 0),
      0
    );
    const otherFeesTotal = otherFees.split(",").reduce((acc, fee) => {
      const [_, amount] = fee.split(":");
      return acc + (parseFloat(amount?.trim()) || 0);
    }, 0);
  ...
  const handleListingSelect = (e) => {
    ...
    if (selectedListing) {
      ...
      if (selectedListing.utilitiesIncluded) {
        setUtilities(
          selectedListing.includedUtilities.map((util) => ({
            name: util,
            fee: 0,
            selected: true,
          }))
        ...
        >
              ...
              {usePlaceholderTenant ? (
                ...
                <div>
                  <input
                    ...
                    onChange={handleChange}
                    placeholder={
                      passedSeekerId?._id || "Enter Tenant ID or use placeholder"
                    }
                    className={`mt-1 block w-full border rounded-md shadow-sm sm:text-sm px-4 py-2 ${
                      darkMode
                        ? "bg-gray-700 border-gray-600 text-white focus:ring-blue-500 focus:border-blue-500"
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                <label
                  className={`block text-sm playlist-medium ${
                    darkMode ? "text-gray-300" : "text-gray-700"
                  }`}
                >
                  Lease End Date
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {utilities.map((util, index) => (
                  <div key={index} className="flex items-center space-x-2 mb-2">
                    <input
                      ...
                      className={`mt-1 block border rounded-md shadow-sm sm:text-sm ${
                        darkMode
                          ? "bg-gray-700 border-gray-600 text-white focus:ring-blue-500 focus:border-blue-500"
                          : "bg-white border-gray-300 text-black focus:ring-blue-500 focus:border-blue-500"
                      }`}
                    ...
                    {util.name ? (
                      ...
                      >
                        {util.name.charAt(0).toUpperCase() + util.name.slice(1)}
                      </span>
                    ) : (
                      <input
                        type="text"
              ...
              <div className="col-span-2">
                ...
                >
                  Other Fees (format: Name:Amount, Name:Amount)
                </label>
                <textarea
